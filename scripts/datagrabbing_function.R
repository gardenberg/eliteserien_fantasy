...
datagrabber = function(x = "spillere"){
        ...
        if(x=="spillere"){
                ...
                #lagrer to lokale kopier av dataene
                write.csv2(df,paste0("data/spillerdata_",Sys.Date(),".csv"),row.names=F)
                write.csv2(df,paste0("data/old/spillerdata_",Sys.Date(),".csv"),row.names=F)
                #returnerer df
                return(df)
        }
        if(x=="spillere_poeng_kamp"){
                ...
                temp_id_spillere = as.vector(temp_df$id)
                data_df =data.frame()
                warning(paste0(Sys.time(),":  Totalt ", length(temp_id_spillere)," spillere. Skraping vil ta ca. ",ceiling((length(temp_id_spillere)*3)/60)," minutter"))
                for(i in temp_id_spillere){
                        Sys.sleep(3)
                ...
                write.csv2(data_df,paste0("data/spillere-poeng_",Sys.Date(),".csv"),row.names=F)
